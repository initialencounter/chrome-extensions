 ...
 * @returns {any}
 */
export function check_pek_bty(data) {
    const ret = wasm.check_pek_bty(data);
    if (ret[2]) {
 ...
 * @returns {any}
 */
export function check_sek_bty(data) {
    const ret = wasm.check_sek_bty(data);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('validators_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
